...
const useRawFetch = createFetch({
  ...
  options: {
    async beforeFetch(ctx) {
      ...
      if (options.prefix)
        ctx.url = ctx.url.replace(API_PREFIX, options.prefix)
      // options.headers = {
      //   ...options.headers
...
export function useFetch() {
  async function get<T>(url: string, params?: Record<string, any>, requestInit: UseRequestInit = {}, options: UseFetchOptions = {}) {
    const { data } = await useRawFetch<T>(normalizeUrl(url, params), requestInit, options).get().json<T>()
    return unref(data)
  }

  async function post<T>(url: string, params?: Record<string, any>, requestInit: UseRequestInit = {}, options: UseFetchOptions = {}) {
